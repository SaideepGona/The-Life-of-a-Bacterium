...
func (p *Petri) ReadToInitializeBact(filename string) {
...
func (p *Petri) ReadEnergyResourceFile(filename string) float64 {
     lines :=  ReadFile(filename)
...
// sunlight, temperature, oxygen, carbon
     ...
     if items1[0] != "carbon" {
       fmt.Println("Error happens with the file, line 1.1")
       os.Exit(1)
     }
     carbon,_ := strconv.ParseFloat(items1[1],64)
     if carbon > 100.0 || carbon < 0.0 {
       fmt.Println("Error happens with the file, line 1.2")
       os.Exit(1)
     }
// Continue with the second line in the file
     var items2 []string = strings.Split(lines[1], " ")
     if items2[0] != "minerals" {
       fmt.Println("Error happens with the file, line 2.1")
       os.Exit(1)
     }
     minerals,_ := strconv.ParseFloat(items2[1],64)
     if minerals > 100.0 || minerals < 0.0  {
       fmt.Println("Error happens with the file, line 2.2")
       os.Exit(1)
     }
// continue with the third line
     var items3 []string = strings.Split(lines[2], " ")
     if items3[0] != "pentose" {
       fmt.Println("Error happens with the file, line 3.1")
       os.Exit(1)
     }
     pentose,_ := strconv.ParseFloat(items3[1],64)
     if pentose > 100.0 || pentose < 0.0  {
       fmt.Println("Error happens with the file, line 3.2")
       os.Exit(1)
     }
// 4th line
    var items4 []string = strings.Split(lines[3], " ")
    if items4[0] != "sunlight" {
        fmt.Println("Error happens with the file, line 4.1")
        os.Exit(1)
      }
     sunlight,_ := strconv.ParseFloat(items4[1],64)
     if sunlight > 100.0 || sunlight < 0.0  {
       fmt.Println("Error happens with the file, line 4.2")
       os.Exit(1)
     }
// 5 th line
    var items5 []string = strings.Split(lines[4], " ")
    if items5[0] != "temperature" {
        fmt.Println("Error happens with the file, line 5.1")
        os.Exit(1)
      }
     temperature,_ := strconv.ParseFloat(items5[1],64)
     if temperature > 100.0 || temperature < 0.0  {
       fmt.Println("Error happens with the file, line 5.2")
       os.Exit(1)
     }
// 6th line
    var items6 []string = strings.Split(lines[5], " ")
    if items6[0] != "oxygen" {
        fmt.Println("Error happens with the file, line 6.1")
        os.Exit(1)
      }
     oxygen,_ := strconv.ParseFloat(items6[1],64)
     if oxygen > 100.0 || oxygen < 0.0  {
       fmt.Println("Error happens with the file, line 6.2")
       os.Exit(1)
     }

     for i := range p.allBacteria{
     p.allBacteria[i].currentEnergy = (carbon+minerals+pentose)
     p.allBacteria[i].stepSize = (sunlight + temperature + oxygen)/32.0 + 0.5
   }
   return sunlight
}
/*
