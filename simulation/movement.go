// Code by Zhenyu Yang, Xinling Li, Kwanho Kim
...
/*This function checkes if there is complete overlap among (x,y) and the location of all the bacteria in the slice*/
...
/*This function checkes if there is partial overlap among (x,y) and the location of all the predator in the slice*/
...
/*this function is about predator movement*/
func (p *Petri) MoveToBacteria() {
	// first, range through all the predators in the petri dish
	for i := 0; i < len(p.allPredator); i++ {
		if p.IsBacteriaAround(i) == true && p.allPredator[i].hasRep == false {
			...
			// k and minDistance stands for which predators were detected and how much
			// distance it has.
