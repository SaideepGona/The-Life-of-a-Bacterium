...
func (pred *Predator) CanReplicate(p *Petri, initialPred int) bool {
	// This function tests whether a bacterium has enough energy to replicate.
	// If it has energyCap greater than repEnergy, the function returns true.
	if pred.currentEnergy > pred.repEnergy && pred.IsThereSpace(p) && len(p.allPredator) < 2*initialPred {
		return true
	}
	return false
}

// Replication only occurs when there is enough space nearby a parent
...
func (p *Petri) Replication(initialPred int) {
	...
	for i := 0; i < numPred; i++ {
		...
		if pred.CanReplicate(p, initialPred) {
...
// Picks a location using a subroutine and appends to the existing slice of bacteria
...
// Choose replication location right next to parent
...
// Similar to bacteria initialization in Initialization.go but this time,
// genetic material (DNA) gets copied from parent.
