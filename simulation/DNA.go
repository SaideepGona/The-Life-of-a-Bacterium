...
func (dna *DNA) MakeConfig(currentLine []string) {
	...
	/*
	If a given line of input is part of the "Configuration" section of read-in file, this function handles its assignment to 
	the DNA object being created.
	*/
...
func (dna *DNA) MakePhen(currentLine []string) {
	...
	phenotype.aggFunction = currentLine[1]				// Tells what kind of phenotype is bieng handled
	phenotype.aggFuncArgs = currentLine[2:]				// Stores all arguments for multi-argument downstream functions
...
func (dna *DNA) MakeGene(currentLine []string) {
	...
	} else if currentLine[1] == "lk" {					// Handles lock and key gene differently
...
func (dna *DNA) MakeEdge(currentLine []string) {
	...
	if len(currentLine) != 4 {												// Enforces number of arguments for an edge
	...
	dna.edges[edgeName] = edge												// Addes edge to edge map
...
func (p *Petri) AllPhenotypeExpectation(phenotypeName string) float64 {
	...
	// Loops through all current bacteria and calculates the mean expectation for that phenotype across all bacteria
...
func (dna *DNA) PhenotypeExpectation(phenotypeName string) float64 {
	...
	// For a single bacteria, probes genes for the phenotype of interest. Calculates phenotype value as if the sample size is the size
	// of the gene (sample means are each population means)
...
func (dna *DNA) PhenotypeAverage(phenotypeName string) float64 {
	...
	/*
	For a single bacteria, probes genes for the phenotype of interest. Calculates phenotype values using the given sample size
	*/
